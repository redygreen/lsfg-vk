...
- **Adaptive Frame Generation / `adaptive`**: When enabled, lsfg-vk chooses how many extra frames to generate **during the game's present**, using the same WSI path as Fixed mode (required on Steam Deck / XWayland, where presenting from a second thread black-screens). The count is `round(target_fps * real_interval - 1)`, capped by `multiplier`. Extra frames are interpolated at `(i+1)/(genCount+1)`. Adaptive cannot reduce a game already rendering above the target. Restart after toggling Adaptive. (Default: `false`)
...
The "Multiplier", "Adaptive", "Target FPS", "Flow Scale" and "Performance Mode" options can be **hot-reloaded**, meaning that changes to these options will take effect immediately without needing to restart the application. Restart after changing the multiplier ceiling so the swapchain has enough image capacity. Switching Adaptive on or off also needs a game restart so the present path is created correctly. Options such as "Pacing Mode" or removal of the profile require a swapchain recreation, which usually means resizing or restarting the application. Any other change requires an application restart. 
