...
- **Multiplier / `multiplier`**: The frame generation multiplier. A value of 3 means that for every frame rendered by the application, lsfg-vk will generate 2 additional frames. When Adaptive mode is enabled, this value is the **maximum** multiplier (ceiling) rather than a fixed rate. (Default: `2`)
- **Adaptive Frame Generation / `adaptive`**: When enabled, lsfg-vk presents on an independent `target_fps` clock. The game renders into a virtual swapchain and is not blocked by generated presents. Each output tick interpolates between the last two real frames at `alpha = elapsed / real_interval`. `multiplier` is only a ceiling on generated frames per real interval. Adaptive cannot reduce a game already rendering above the target; extra real frames are simply not displayed. Restart the application after switching between Fixed and Adaptive so the swapchain is created with virtual images. (Default: `false`)
- **Adaptive Target / `target_fps`**: Desired displayed framerate for Adaptive mode. Cap the game separately if you want its render rate limited. (Default: `60`)
...
The "Multiplier", "Adaptive", "Target FPS", "Flow Scale" and "Performance Mode" options can be **hot-reloaded**, meaning that changes to these options will take effect immediately without needing to restart the application. Restart after changing the multiplier ceiling so the swapchain has enough image capacity. Options such as "Pacing Mode" or removal of the profile require a swapchain recreation, which usually means resizing or restarting the application. Any other change requires an application restart. 
...
- `LSFGVK_MULTIPLIER`: Frame generation multiplier (max/ceiling when adaptive).
- `LSFGVK_ADAPTIVE`: If set to `1`, Adaptive Frame Generation is enabled.
