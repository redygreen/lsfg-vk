...
- **Adaptive Frame Generation / `adaptive`**: When enabled, lsfg-vk measures the real-frame interval and generates only as many intermediate frames as needed to approach `target_fps`. The count can change every real frame (including fractional averages via error accumulation). Fixed `multiplier` is ignored as a rate and used only as a ceiling. Adaptive cannot reduce a game already rendering above the target. (Default: `false`)
- **Adaptive Target / `target_fps`**: Desired displayed framerate for Adaptive mode. Cap the game separately if its real framerate can exceed this value. (Default: `60`)
